...
using TheSite.ManutenzioneProgrammata;
using StampaRapportiPdf.Classi;


#region   AGGIORNAMENTO
/*   Questa pagina non è completa ne totalmente funzionante, le attività sono state interrotte
 *	 perche è un attività che il cliente non richiede più!  (04/09/2006)
 * 
 * */
#endregion
...
{
	/// <summary>
	/// Descrizione di riepilogo per SfogliaComplete.
	/// </summary>
	public class SfogliaComplete : System.Web.UI.Page    // System.Web.UI.Page
	{
...
//		protected System.Web.UI.WebControls.DataGrid DataGridRicerca2;
		...
		protected S_Controls.S_ComboBox cmbApprovate;
...
//			}
		...
		//  funzione in cui modificare i parametri 
		private DataSet GetData()
		{
			...
			s_p_Status.Index = 8;
			// ora il valore "Approvate" è fisso
			s_p_Status.Value =   4 ; // (cmbsStatus.SelectedValue ==string.Empty)? 0:Int32.Parse(cmbsStatus.SelectedValue);			
			_SCollection.Add(s_p_Status);

			// Nuovo parametro per le Wr Approvate
			S_Controls.Collections.S_Object s_p_Approvate = new S_Controls.Collections.S_Object();
			s_p_Approvate.ParameterName = "p_Approvate";
			s_p_Approvate.DbType = ApplicationDataLayer.DBType.CustomDBType.Integer;
			s_p_Approvate.Direction = ParameterDirection.Input;
			s_p_Approvate.Index = 9; 
			s_p_Approvate.Value =  (cmbApprovate.SelectedValue ==string.Empty)? 0:Int32.Parse(cmbApprovate.SelectedValue);	
			_SCollection.Add(s_p_Approvate);
			// fine nuovo parametro
			...
			DataSet _MyDs = _Richiesta.GetSfogliaRDLComplete(_SCollection).Copy();	
			return _MyDs;
		}
		...
		private void DataGridRicerca_ItemDataBound(object sender, System.Web.UI.WebControls.DataGridItemEventArgs e)
		{
			...
			{ 
				...
				System.Web.UI.HtmlControls.HtmlAnchor  _linkPre = (System.Web.UI.HtmlControls.HtmlAnchor) e.Item.Cells[3].Controls[1];
				 System.Web.UI.HtmlControls.HtmlAnchor  _linkCon = (System.Web.UI.HtmlControls.HtmlAnchor) e.Item.Cells[5].Controls[1];
		...
		//	Gridtitle2.Visible=true;
			...
			s_p_Status.Direction = ParameterDirection.Input;
			s_p_Status.Index = 8;
	// TO DO 	s_p_Status.Value = (cmbsStatus.SelectedValue ==string.Empty)? 0:Int32.Parse(cmbsStatus.SelectedValue);			
			_SCollection.Add(s_p_Status);
...
//			this.Gridtitle2.NumeroRecords = _MyDs.Tables[0].Rows.Count.ToString();	
	...
	// TO DO		s_p_Status.Value = (cmbsStatus.SelectedValue ==string.Empty)? 0:Int32.Parse(cmbsStatus.SelectedValue);			
		...
		// Paolo (gestione selezione multipla)  RIPRENDO DA QUI (..la classe che contiene i dati??  ..sentire Fabio)
...
//		}
		...
		{	
			...
			foreach(DataGridItem o_Litem in DataGridRicerca.Items)
			{
				// Devo cambiare il contenitore dei dati dove salvare i record
				//  DataGridField _campi = new DataGridField();

				DGridRdlComplete _campi = new DGridRdlComplete();
		
				_campi.WrId =Int32.Parse(o_Litem.Cells[0].Text);									
				System.Web.UI.WebControls.CheckBox cb = (System.Web.UI.WebControls.CheckBox) o_Litem.Cells[4].FindControl("ChkSel");																									
				cb.Checked=val;
				if(_HS.ContainsKey(_campi.WrId))
					_HS.Remove(_campi.WrId);												
							
				if(cb.Checked)
				{
					_campi.WrId						= Int32.Parse(o_Litem.Cells[4].Text);
					_campi.OdlId					= Int32.Parse(o_Litem.Cells[5].Text);
					_campi.impianto					= o_Litem.Cells[6].Text;
					_campi.addetto					= o_Litem.Cells[7].Text;
					_campi.stato						= o_Litem.Cells[8].Text;
					_campi.tipoInt					= o_Litem.Cells[9].Text;
					_campi.impStimato				= o_Litem.Cells[10].Text;
					_campi.impConsuntivo	= o_Litem.Cells[10].Text;
					_HS.Add(_campi.WrId, _campi);		
				}
			}			
		...
		private void btnsDeSelezionaTutti_Click(object sender, System.EventArgs e)
		{
			SelezionaTutti(false);
				...
				// Paolo (FINE gestione selezione multipla)
