...
namespace TheSite.ManutenzioneCorretiva
{
	...
	public class ModificaRdl : System.Web.UI.Page    // System.Web.UI.Page
	{
		...
		TheSite.ManutenzioneCorrettiva.SfogliaRdl _fp;
		public int ItemId=0;
		...
		private void Page_Load(object sender, System.EventArgs e)
		{
			...
			string _mypage="./ManutenzioneCorrettiva/ModificaRdl.aspx";			
			Classi.SiteModule _SiteModule = new TheSite.Classi.SiteModule(_mypage);
			// ***********************  MODIFICA PER I PERMESSI SULLA PAGINA CORRENTE **********************

			FunId = _SiteModule.ModuleId;
			HelpLink = _SiteModule.HelpLink;
			this.PageTitle1.Title = "Modifica Rdl n°: "+ItemId.ToString();//_SiteModule.ModuleTitle;
			...
			if (!Page.IsPostBack)
			{	
				btnsSalva.Visible=true;
				
				if(Context.Handler is TheSite.ManutenzioneCorrettiva.SfogliaRdl) 
				{
					_fp = (TheSite.ManutenzioneCorrettiva.SfogliaRdl) Context.Handler;
			...
			//	RFVStanza.ControlToValidate=UserStanze1.ID+":"+UserStanze1.s_TxtIdStanza.ID;
			...
			if (_Richiesta.IsValidBlId(this.RicercaModulo1.BlId))
			{
				...
				//	Response.Redirect("VisualizzaRdl.aspx?FunId=" + FunId + "&ItemId=" + i_Wr_Id.ToString());
					if (ViewState["PAG"].ToString()=="SF")			
					{
						...
						Server.Transfer("SfogliaRdl.aspx");
					}
					else
					{
...
//			cmbsStanza.Attributes.Add("OnChange","ClearApparechiature();");
		...
		{	int i_WrId=0;
			...
			p_id_stanza.Direction = ParameterDirection.InputOutput;
		...
		private void BindApparecchiatura(string CodEdificio)
		{
			...
			else
			{
				...
				this.cmbsApparecchiatura.Items.Add(Classi.GestoreDropDownList.ItemMessaggio(s_Messagggio, ""));
			}
		...
		private void BindServizio(string CodEdificio)
		{
			...
			string s_Messagggio1 = "Non Definito";
			cmbsServizio.Items.Add(Classi.GestoreDropDownList.ItemMessaggio(s_Messagggio1, "0"));
