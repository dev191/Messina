...
// Type: TheSite.ManutenzioneCorrettiva.CreazioneSGA1
...
{
  public class CreazioneSGA1 : Page
  {
    ...
    {
      SiteModule siteModule = new SiteModule("./ManutenzioneCorrettiva/CreazioneSGA1.aspx");
      CreazioneSGA1.FunId = siteModule.ModuleId;
      CreazioneSGA1.HelpLink = siteModule.HelpLink;
      ...
      if (!this.Page.IsPostBack)
      {
        ...
        TextBox txtsorain = this.txtsorain;
        DateTime now = DateTime.Now;
        string str1 = Convert.ToString(now.Hour).PadLeft(2, '0');
        txtsorain.Text = str1;
        TextBox txtsorainmin = this.txtsorainmin;
        now = DateTime.Now;
        string str2 = Convert.ToString(now.Minute).PadLeft(2, '0');
        txtsorainmin.Text = str2;
    ...
    private void LoadTipoManutenzione()
    {
      ...
      if (this.Request["VarApp"] == null || !(this.Request["VarApp"] == "1"))
        return;
      this.cmbsTipoManutenzione.SelectedValue = "3";
    }
    ...
    private void BindServizio(string CodEdificio)
    {
      ...
      if (CodEdificio != string.Empty)
      {
        ...
        CreazioneSGA1.myEnumerator = hashtable.GetEnumerator();
    ...
    private void btnsSalva_Click(object sender, EventArgs e)
    {
      if (new Richiesta().IsValidBlId(this.RicercaModulo1.BlId))
      {
        ...
        if (this.ItemId == 0)
        {
          ...
          this.Response.Redirect("VisualizzaRdl1.aspx?FunId=" + (object) CreazioneSGA1.FunId + "&ItemId=" + num.ToString() + str);
        }
