...
// Type: TheSite.CommonPage.ListaStanzeRic
...
{
  public class ListaStanzeRic : Page
  {
    ...
    private void Execute(bool reset)
    {
      ...
      this.MyDataGrid1.DataSource = (object) apparecchiature.RicercaDescStanze(CollezioneControlli1);
      if (reset)
      {
        ((CollectionBase) CollezioneControlli1).Clear();
        S_ControlsCollection CollezioneControlli2 = this.getParam();
        this.GridTitle1.NumeroRecords = apparecchiature.RicercaStanzeDescCount(CollezioneControlli2).ToString();
      }
