...
// Type: TheSite.CommonPage.SchedaApparecchiatura
...
namespace TheSite.CommonPage
{
  public class SchedaApparecchiatura : Page
  {
    ...
    protected S_Label S_LblStato;
    private int IDEQ = 0;

    private void Page_Load(object sender, EventArgs e)
    {
      SiteModule siteModule = (SiteModule) HttpContext.Current.Items[(object) "SiteModule"];
      SchedaApparecchiatura.FunId = siteModule.ModuleId;
      SchedaApparecchiatura.HelpLink = siteModule.HelpLink;
    ...
    private void Execute()
    {
      ...
      DataSet data = new TheSite.Classi.ClassiDettaglio.SchedaApparecchiatura("").GetData(CollezioneControlli);
      if (data.Tables[0].Rows.Count > 0)
      {
        ...
        try
        {
          ...
          ((Label) this.S_LblStato).Text = data.Tables[0].Rows[0]["var_eq_condition"].ToString();
        }
        catch (Exception ex)
        {
