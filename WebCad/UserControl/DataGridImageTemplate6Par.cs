...
// Type: WebCad.UserControl.DataGridImageTemplate6Par
...
namespace WebCad.UserControl
{
  public class DataGridImageTemplate6Par : ITemplate
  {
    ...
    public DataGridImageTemplate6Par(
      ListItemType type,
      string testoColonna,
      string colname1,
      string colname2,
    ...
    private void BindDataLiteral(object sender, EventArgs e)
    {
      ...
      htmlImage.Attributes.Add("onClick", this.commandImage.Split('$')[0] + ((DataRowView) namingContainer.DataItem)[this.columnName1].ToString().Replace(",", ".") + ", " + ((DataRowView) namingContainer.DataItem)[this.columnName2].ToString().Replace(",", ".") + ", " + ((DataRowView) namingContainer.DataItem)[this.columnName3].ToString().Replace(",", ".") + ", " + ((DataRowView) namingContainer.DataItem)[this.columnName4].ToString().Replace(",", ".") + " " + ((DataRowView) namingContainer.DataItem)[this.columnName5].ToString().Replace(",", ".") + " " + ((DataRowView) namingContainer.DataItem)[this.columnName6].ToString().Replace(",", ".") + " " + this.commandImage.Split('$')[1]);
    }
  }
}
