...
// Type: TheSite.Gestione.EditSettori
...
namespace TheSite.Gestione
{
  public class EditSettori : Page
  {
    ...
    private TheSite.Classi.ClassiAnagrafiche.Settori _Settori = new TheSite.Classi.ClassiAnagrafiche.Settori();
    ...
    private void Page_Load(object sender, EventArgs e)
    {
      ...
      if (this.itemId != 0)
      {
        this._MyDs = this._Settori.GetSingleData(this.itemId);
        if (this._MyDs.Tables[0].Rows.Count == 1)
        {
          DataRow row = this._MyDs.Tables[0].Rows[0];
          if (row["codice_settore"] != DBNull.Value)
            ((TextBox) this.txtsCodice).Text = (string) row["codice_settore"];
          ...
          this.lblOperazione.Text = "Modifica Settore: " + ((TextBox) this.txtsCodice).Text;
          this.lblFirstAndLast.Visible = true;
          ((WebControl) this.btnsElimina).Attributes.Add("onclick", "return confirm('Si vuole effettuare la cancellazione?')");
          this.lblFirstAndLast.Text = this._Settori.GetFirstAndLastUser(row);
        }
      }
      else
      {
        this.lblOperazione.Text = "Inserimento Settore";
      ...
      if (!(this.Context.Handler is Settori))
        return;
      this._fp = (Settori) this.Context.Handler;
    ...
    private void btnsSalva_Click(object sender, EventArgs e)
    {
      ...
      try
      {
        int num = this.itemId != 0 ? this._Settori.Update(this._SCollection, this.itemId) : this._Settori.Add(this._SCollection);
        if (num > 0 && num != -11)
          this.Server.Transfer("Settori.aspx");
        else
    ...
    private void btnsElimina_Click(object sender, EventArgs e)
    {
      try
      {
        ...
        this.txtsNote.set_DBDefaultValue((object) DBNull.Value);
        this._SCollection.AddItems((object) this.PanelEdit.Controls);
        switch (this._Settori.Delete(this._SCollection, this.itemId))
        {
          case -5:
    ...
    private void btnAnnulla_Click(object sender, EventArgs e) => this.Server.Transfer("Settori.aspx");
  }
}
