...
// Type: TheSite.Gestione.EditDestUso
...
namespace TheSite.Gestione
{
  public class EditDestUso : Page
  {
    ...
    private TheSite.Classi.ClassiAnagrafiche.DestUso _DestUso = new TheSite.Classi.ClassiAnagrafiche.DestUso();
    ...
    private void Page_Load(object sender, EventArgs e)
    {
      ...
      if (this.itemId != 0)
      {
        this._MyDs = this._DestUso.GetSingleData(this.itemId);
        if (this._MyDs.Tables[0].Rows.Count == 1)
        {
          DataRow row = this._MyDs.Tables[0].Rows[0];
          if (row["codice_dest_uso"] != DBNull.Value)
            ((TextBox) this.txtsCodice).Text = (string) row["codice_dest_uso"];
          ...
          this.lblOperazione.Text = "Modifica Destinazione d'Uso: " + ((TextBox) this.txtsCodice).Text;
          this.lblFirstAndLast.Visible = true;
          ((WebControl) this.btnsElimina).Attributes.Add("onclick", "return confirm('Si vuole effettuare la cancellazione?')");
          this.lblFirstAndLast.Text = this._DestUso.GetFirstAndLastUser(row);
        }
      }
      else
      {
      ...
      if (this.Request["TipoOper"] == "read")
      {
        ...
        this.lblOperazione.Text = "Visualizzazione Settore: " + ((TextBox) this.txtsCodice).Text;
      }
      this.ViewState["UrlReferrer"] = (object) this.Request.UrlReferrer.ToString();
      if (!(this.Context.Handler is DestUso))
        return;
      this._fp = (DestUso) this.Context.Handler;
    ...
    private void btnsSalva_Click(object sender, EventArgs e)
    {
      ...
      try
      {
        int num = this.itemId != 0 ? this._DestUso.Update(this._SCollection, this.itemId) : this._DestUso.Add(this._SCollection);
        if (num > 0 && num != -11)
          this.Server.Transfer("DestUso.aspx");
        else
          SiteJavaScript.msgBox(this.Page, "La destinazione é stata già inserita");
      }
      catch (Exception ex)
      {
    ...
    private void btnsElimina_Click(object sender, EventArgs e)
    {
      try
      {
        ...
        this.txtsNote.set_DBDefaultValue((object) DBNull.Value);
        this._SCollection.AddItems((object) this.PanelEdit.Controls);
        switch (this._DestUso.Delete(this._SCollection, this.itemId))
        {
          case -5:
    ...
    private void btnAnnulla_Click(object sender, EventArgs e) => this.Server.Transfer("DestUso.aspx");
  }
}
