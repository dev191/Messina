...
namespace TheSite.Gestione
{
	...
	/// </summary>
	public class EditReparti : System.Web.UI.Page    // System.Web.UI.Page
	{
		...
		TheSite.Gestione.Reparti _fp;
		DataSet _MyDs = new DataSet();
		S_Controls.Collections.S_ControlsCollection _SCollection = new S_Controls.Collections.S_ControlsCollection();
		Classi.ClassiAnagrafiche.Reparti _Reparti = new TheSite.Classi.ClassiAnagrafiche.Reparti();
		...
		private void Page_Load(object sender, System.EventArgs e)
		{
			...
			{			
				if (itemId != 0) 
				{
					_MyDs = _Reparti.GetSingleData(itemId); 
					
					if (_MyDs.Tables[0].Rows.Count == 1)
					{					
						DataRow _Dr = _MyDs.Tables[0].Rows[0];
						
						if (_Dr["codice_reparto"] != DBNull.Value)
							this.txtsCodice.Text= (string) _Dr["codice_reparto"];					
						...
						this.lblOperazione.Text = "Modifica Reparto: " + this.txtsCodice.Text;
						this.lblFirstAndLast.Visible = true;			
						this.btnsElimina.Attributes.Add("onclick", "return confirm('Si vuole effettuare la cancellazione?')");	
						lblFirstAndLast.Text = _Reparti.GetFirstAndLastUser(_Dr);
					}
				}
				else
				{
				...
				if (Request["TipoOper"] == "read")	
				{
					...
					this.lblOperazione.Text = "Visualizzazione Reparto: " + this.txtsCodice.Text;
				}
				ViewState["UrlReferrer"] = Request.UrlReferrer.ToString();
				if(Context.Handler is TheSite.Gestione.Reparti) 
				{
					_fp = (TheSite.Gestione.Reparti) Context.Handler;
		...
		private void btnsSalva_Click(object sender, System.EventArgs e)
		{
			...
			{
				if (itemId == 0)
					i_RowsAffected = _Reparti.Add(_SCollection);  
				else
					i_RowsAffected =  _Reparti.Update(_SCollection,itemId);
				
				if ( i_RowsAffected > 0 && i_RowsAffected!=-11)
					Server.Transfer("Reparti.aspx");					
				else
					Classi.SiteJavaScript.msgBox(this.Page,"Il Reparto  é stato già inserito");
			}
			catch (Exception ex)
			{				
		...
		private void btnsElimina_Click(object sender, System.EventArgs e)
		{
			...
			{				
				...
				_SCollection.AddItems(this.PanelEdit.Controls);	
				i_RowsAffected = _Reparti.Delete(_SCollection, itemId);
				...
				switch (i_RowsAffected)
				{
					case-1:
						...
						break;
					case-5:
						PanelMess.ShowError("Impossibile eliminare il reparto in quanto associato ad una stanza", true);
						break;
				}
