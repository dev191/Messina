...
// Type: TheSite.Gestione.TipoIntervento
...
{
  public class TipoIntervento : Page
  {
    ...
    private EditTipoIntervento _fp;

    private void Page_Load(object sender, EventArgs e)
    {
      SiteModule siteModule = (SiteModule) HttpContext.Current.Items[(object) "SiteModule"];
      ((HyperLink) this.GridTitle1.hplsNuovo).NavigateUrl = "../Gestione/EditTipoIntervento.aspx?ItemID=0&FunId=" + (object) siteModule.ModuleId;
      ...
      TipoIntervento.FunId = siteModule.ModuleId;
      TipoIntervento.HelpLink = siteModule.HelpLink;
      this.PageTitle1.Title = siteModule.ModuleTitle;
      if (this.Page.IsPostBack || !(this.Context.Handler is EditTipoIntervento))
        return;
      this._fp = (EditTipoIntervento) this.Context.Handler;
      if (this._fp == null)
        return;
    ...
    private void Ricerca()
    {
      ...
      this.txtsDescrizioneBreve.set_DBDefaultValue((object) "%");
      ...
      DataSet dataSet = tipoIntervento.GetData(CollezioneControlli).Copy();
    ...
    private void BtnReset_Click(object sender, EventArgs e) => this.Response.Redirect("TipoIntervento.aspx?FunId=" + (object) TipoIntervento.FunId);
  }
}
